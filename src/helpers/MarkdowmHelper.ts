import {
  Collection, CollectionItem, CollectionRequest,
  CollectionRequestOptionBody, CollectionRequestOptionHeader,
...
class MarkdowmHelper {
  ...
  private static transformRequestHeaders(headers: Array<CollectionRequestOptionHeader>): string {
    ...
    return (!headers.length)
      ? ''
      : `${tableHeader
        + headers.reduce((previous: string, header: CollectionRequestOptionHeader): string => `${previous}|${header.key}|${header.value}|\n`, '')
      }\n`;
  }
